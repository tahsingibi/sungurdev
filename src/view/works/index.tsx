...
function ProjectRow({ project }: { project: Project }) {
  ...
  return (
    <li className="group relative flex flex-col gap-4 p-6 transition-colors hover:bg-accent/40 sm:flex-row sm:items-start sm:justify-between last:[&_.divider-b]:hidden!">
      <div className="flex min-w-0 gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-2 font-pixel text-xs upp text-muted-foreground space-x-2">
  ...
  return (
    <div className="relative flex flex-col">
      ...
      {experience.map((item, experienceIndex) => {
        ...
        return (
          <section key={item.id} id={anchor} className="relative scroll-mt-24">
            {experienceIndex > 0 ? (
              <div
                className="divider-screen divider-hatch"
                aria-hidden="true"
              />
            ) : null}
            ...
            <header className="relative flex flex-col gap-1 p-6">
              <div className="flex items-baseline justify-between gap-4">
                ...
                <time className="shrink-0 font-pixel text-xs upp text-zinc-400">
                  {item.year}
                </time>
              </div>
              <div className="flex items-center justify-between gap-4">
                <p className="font-pixel text-sm upp text-foreground/70">
                  {item.title}
                </p>
                <Link
                  href={item.path}
                  aria-label={`${item.name} bölümünün bağlantısı`}
                  className="font-pixel text-[10px] upp text-muted-foreground transition-colors hover:text-foreground"
                >
            ...
            <div
              ...
              className="p-6 text-foreground/75 [&_.mark]:font-black [&_a]:underline [&_a]:underline-offset-4 [&_li+li]:mt-2 [&_li]:pl-1 [&_p+p]:mt-4 [&_ul]:mt-4 [&_ul]:list-disc [&_ul]:pl-5"
            />
